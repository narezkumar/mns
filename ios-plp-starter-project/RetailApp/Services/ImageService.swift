...
class ImageServiceImplementation: ImageService {
  ...
  func downloadImage(key: String, completion: @escaping (Result<UIImage, Error>) -> Void) {
    let resource = Resource(path: "image/\(key).jpg", body: nil, method: "GET", basicAuth: BasicAuthDetails(username: Environment.username, password: Environment.password)) { data -> Result<UIImage, Error> in
